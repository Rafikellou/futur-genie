...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create quiz using service role to bypass RLS
    ...
    if (quizError) {
      ...
      return NextResponse.json({ error: 'Failed to create quiz: ' + quizError.message }, { status: 500 })
    }

    return NextResponse.json({ quiz }, { status: 201 })
  } catch (e: any) {
    console.error('Unexpected error in quizzes/create:', e)
