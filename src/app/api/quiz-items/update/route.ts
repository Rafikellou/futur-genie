...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'Missing quiz item ID' }, { status: 400 })
    }

    // Update quiz item using service role to bypass RLS
    ...
    if (itemError) {
      console.error('Quiz item update error:', itemError)
      return NextResponse.json({ error: 'Failed to update quiz item: ' + itemError.message }, { status: 500 })
    }

    return NextResponse.json({ quizItem }, { status: 200 })
  } catch (e: any) {
    console.error('Unexpected error in quiz-items/update:', e)
